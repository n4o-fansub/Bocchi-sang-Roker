...
def ass_hex_to_hls(ass_hex):
    return colorsys.rgb_to_hls(*ass_hex_to_rgb(ass_hex))
...
def desaturate_hsl(hsl):
    h, s, l = hsl
    # Using the example, try to emulate it
    return (h * 0.001, s * 0.11, l * 0.66)
...
for sub in subs.events:
    ...
    if (mm := onec_regex.search(sub.text)):
        ...
        sub.text = onec_regex.sub(
            r"\\1c&H\1&\\t(0,3939,1.5,\\1c&H" + target_ass + r"&)", sub.text
        )
        print(sub.text)
        total_lines += 1
...
print(total_lines)
ass_parser.write_ass(subs, path.with_stem("bocchi02.ts.desaturate"))
